...
```{r}
titanic<-data.frame(Titanic) # created a dataframe object 
```
...
```{r}
nchi<-sum(titanic[titanic[,3]=="Child",5]) # calculated the total number of children 
nadu<-sum(titanic[titanic[,3]=="Adult",5]) # calculated the total number of adults
```
...
```{r}
adult<-titanic[titanic[,3]=="Adult",]
nma<-sum(adult[adult[,2]=="Male",5])
nfe<-sum(adult[adult[,2]=="Female",5])
x<-ifelse(nma<nfe, "female", "male")
```
...
```{r}
child<-titanic[titanic[,3]=="Child",]
radu<-sum(adult[adult[,4]=="Yes",5])/sum(adult[,5])
rchi<-sum(child[child[,4]=="Yes",5])/sum(child[,5])
m1<-matrix(data=c(radu, rchi), nrow=1, ncol=2)
colnames(m1)<-c("Adult", "Child")
barplot(height=m1,main = "Adult vs Child Survival", ylab="Survival Rate")
x2<-ifelse(rchi>radu,"did","did not")
```
